...
## Categorical Covariate: Sex & ICU Status
...
```{r}
#calculate median for histogram to determine the geom_vline
median_ABCA1 <- median(ABCA1.gene.exp.meta$ABCA1)
max_ABCA1 <- max(ABCA1.gene.exp.meta$ABCA1)
```
...
ggplot(ABCA1.gene.exp.meta, aes(x=ABCA1.gene.exp.meta$ABCA1)) + 
  ...
  geom_vline(xintercept = median(ABCA1.gene.exp.meta$ABCA1), size = 1, color="red") +
  annotate("text", x=median_ABCA1*1.8, y=0.5*max_ABCA1, label=paste("Median = ", median_ABCA1), color=("black")) +
...
```{r}
#calculating correlation for scatter plot
cor.test(ABCA1.gene.exp.meta$age,ABCA1.gene.exp.meta$ABCA1)
```
